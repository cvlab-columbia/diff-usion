...
def train_transforms(img):
    img = center_crop(img)
    img = img.resize(
        (cfg.resolution, cfg.resolution), resample=Image.BICUBIC, reducing_gap=1
    )
...
def main(cfg: KandinskyLoRAConfig):
    ...
    for num_images in [1000,2000,3000,4000,5000,6000,7000,8000]:
        run_name = "debug"
        ...
        # clf.to(accelerator.device, dtype=weight_dtype)
        ...
        cfg.dataset.num_samples = num_images

        cfg.output_dir = Path(f'output/lora/' + cfg.dataset.name + "_" + str(num_images))
        os.makedirs(cfg.output_dir, exist_ok=True)
        ...
        # create pipeline
        val_pipeline = KandinskyV22Pipeline.from_pretrained(
        ...
        for epoch in range(first_epoch, cfg.num_train_epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                with accelerator.accumulate(lora_net):
                    ...
                    # image_embeds = clf.get_features(images)
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % cfg.checkpointing_steps == 0:
                        if accelerator.is_main_process:
                            ...
                            lora_net.save_pretrained(save_path)
